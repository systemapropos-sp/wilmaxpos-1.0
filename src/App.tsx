...
function App() {
  const {
    isAuthenticated, sidebarCollapsed, currentPage,
    initialized, loading, initializeApp,
    businessId,
  } = useAppStore();
  const { notifications, markAsRead, markAllAsRead, clearNotification } = useNotifications();

  // Only init once businessId is known (persisted from previous login or just selected)
  useEffect(() => {
    if (businessId) initializeApp();
  }, [businessId, initializeApp]);

  // Show spinner only when we have a business but are still loading data
  if (businessId && (loading || !initialized)) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[var(--background)]">
        <div className="text-center">
          <div className="w-16 h-16 border-4 border-[#ED6823] border-t-transparent rounded-full animate-spin mx-auto mb-4" />
  ...
  // Not authenticated (or no businessId yet) — show login
  if (!isAuthenticated) {
    return (
      <>
        <Login />
